package tutorial.rest.resources.asm;

import org.springframework.hateoas.mvc.ResourceAssemblerSupport;
import tutorial.core.services.util.BlogList;
import tutorial.rest.mvc.BlogController;
import tutorial.rest.resources.BlogListResource;

public class BlogListResourceAsm extends ResourceAssemblerSupport<BlogList, BlogListResource> {

    public BlogListResourceAsm()
    {
        super(BlogController.class, tutorial.rest.resources.BlogListResource.class);
    }

    @Override
    public tutorial.rest.resources.BlogListResource toResource(BlogList blogList) {
        tutorial.rest.resources.BlogListResource res = new tutorial.rest.resources.BlogListResource();
        res.setBlogs(new tutorial.rest.resources.asm.BlogResourceAsm().toResources(blogList.getBlogs()));
        return res;
    }
}